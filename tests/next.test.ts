...
describe('getSessionFromNextCookies', () => {
  ...
  it('should throw error when no secret provided', async () => {
    delete process.env.ADAPT_AUTH_SESSION_SECRET;

    const cookies = { get: jest.fn() };

    await expect(getSessionFromNextCookies(cookies)).rejects.toThrow(
      'Session secret is required. Provide it as parameter or set ADAPT_AUTH_SESSION_SECRET environment variable.'
    );
  });

  it('should return null when cookies object has no get method', async () => {
    process.env.ADAPT_AUTH_SESSION_SECRET = 'test-secret-32-chars-long!!!!!';
  ...
  it('should return null when session cookie not found', async () => {
    process.env.ADAPT_AUTH_SESSION_SECRET = 'test-secret-32-chars-long!!!!!';
    delete process.env.ADAPT_AUTH_SESSION_NAME; // Ensure we use the default name
    ...
    expect(result).toBeNull();
    expect(cookies.get).toHaveBeenCalledWith('adapt-auth-session');
  });

  it('should use environment variables for configuration', async () => {
    process.env.ADAPT_AUTH_SESSION_SECRET = 'env-secret-32-chars-long!!!';
    process.env.ADAPT_AUTH_SESSION_NAME = 'custom-session';
  ...
  it('should use provided parameters over environment variables', async () => {
    process.env.ADAPT_AUTH_SESSION_SECRET = 'env-secret';
    process.env.ADAPT_AUTH_SESSION_NAME = 'env-session';
